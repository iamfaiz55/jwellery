...
import { useLoginAdminMutation, useVerifyAdminOTPMutation } from '../../../redux/apis/adminAuthApi';
import { useSelector } from 'react-redux';
const AdminLogin = () => {
    const [otp, setOtp] = useState()
    const navigate = useNavigate()
    const [loginAdmin, { isSuccess, isLoading, isError, error, data }] = useLoginAdminMutation()
    const { admin, mobile } = useSelector(state => state.admin)
    const [verifyOTP, {
    ...
    // const { mobile } = useSelector(state => state.adminData)
                            ...
                            <div className="mb-4">
                                ...
                                </div>
                                {
                                    show
                                        ...
                                        : <h1 className="mb-1 fw-bold">Sign in Admin</h1>
                                }

                                {
                                    show
                                        ? <div>
                                            ...
                                            <Button onClick={e => verifyOTP({ mobile: mobile, otp })} className='w-100' disabled={verifyIsLoading}> {verifyIsLoading ? "Please Wait Verifing.." : "Verify OTP"}</Button>
